...
for (i in c(190000, 142500, 95000, 47500)) { 
     ...
     a <- system.time({traj_milo4 <- miloR::buildGraph(traj_milo, k = 10, d = 30);
     traj_milo4 <- makeNhoods2(traj_milo4, prop = 0.1, k = 10, d=30, refined = TRUE, refinement_scheme = "reduced_dim");
     traj_milo4 <- miloR::countCells(traj_milo4, meta.data = data.frame(colData(traj_milo)), sample="Sample");
     traj_milo4 <- miloR::calcNhoodDistance(traj_milo4, d=30);
     traj_milo4 <- testNhoods2(traj_milo4, design = ~ Condition, design.df = traj_design, fdr.weighting = "k-distance")})

     p_red <- profmem({
        traj_milo4 <- miloR::buildGraph(traj_milo, k = 10, d = 30)
        traj_milo4 <- makeNhoods2(traj_milo4, prop = 0.1, k = 10, d=30, refined = TRUE, refinement_scheme = "reduced_dim")
        traj_milo4 <- miloR::countCells(traj_milo4, meta.data = data.frame(colData(traj_milo)), sample="Sample")
        traj_milo4 <- miloR::calcNhoodDistance(traj_milo4, d=30) 
        traj_milo4 <- testNhoods2(traj_milo4, design = ~ Condition, design.df = traj_design, fdr.weighting = "k-distance")
     ...
     b <- system.time({traj_milo5 <- miloR::buildGraph(traj_milo, k = 10, d = 30);
     traj_milo5 <- makeNhoods2(traj_milo5, prop = 0.1, k = 10, d=30, refined = TRUE, refinement_scheme = "graph");
     traj_milo5 <- miloR::countCells(traj_milo5, meta.data = data.frame(colData(traj_milo)), sample="Sample");
     traj_milo5 <- testNhoods2(traj_milo5, design = ~ Condition, design.df = traj_design, fdr.weighting = "graph-overlap")})

     p_graph <- profmem({
        traj_milo5 <- miloR::buildGraph(traj_milo, k = 10, d = 30)
        traj_milo5 <- makeNhoods2(traj_milo5, prop = 0.1, k = 10, d=30, refined = TRUE, refinement_scheme = "graph")
        traj_milo5 <- miloR::countCells(traj_milo5, meta.data = data.frame(colData(traj_milo)), sample="Sample")
        traj_milo5 <- testNhoods2(traj_milo5, design = ~ Condition, design.df = traj_design, fdr.weighting = "graph-overlap")
     })
     ...
     mem_red[j] <- sum(p_red$bytes)/1000000
     mem_graph[j] <- sum(p_graph$bytes)/1000000
     
     j <- j + 1
 }
 
time.df <- cbind(n_cells, reduced_dim, graph, mem_red, mem_graph)
time.df <- data.frame(time.df) 
write_csv(time.df, "/nfs/research/marioni/akluzer/time_df.csv")

# # same as above, but for different values of k
# reduced_dimk <- rep(NA, 10)
# graphk <- rep(NA, 10)
# k_val <- rep(NA, 10)
# j <- 1
# 
...
#     traj_milo2 <- makeNhoods2(traj_milo, prop = 0.1, k = k, d=30, refined = TRUE, refinement_scheme = "reduced_dim");
#     traj_milo2 <- miloR::countCells(traj_milo2, meta.data = data.frame(colData(traj_milo)), sample="Sample");
...
#     traj_milo1 <- miloR::countCells(traj_milo1, meta.data = data.frame(colData(traj_milo)), sample="Sample");
