...
glmm.sim.listTMB <- lapply(sim.list, glmmWrapperTMB)
...
ggplot(to_plot_f, aes(y = value, x = as.factor(ground_truth), group = analysis)) +
    geom_bar(width = 0.8, stat = "identity", color = "black", position = position_dodge(), aes(fill=analysis)) +
    theme_bw() +
    ...
    scale_fill_manual(values = c("#8da0cb", "grey"), name="Model", labels = c("GLMM", "TMB"))
...
ggplot(to_plot_s, aes(y = value, x = as.factor(ground_truth), group = analysis)) +
    ...
    theme_bw() +
    xlab("Ground truth \u03B21") +
    ylab("Estimated \u03B21 SE") +
    theme(legend.position = "none", axis.text.x=element_text(colour="black"), axis.text.y=element_text(colour="black")) +
    theme(text = element_text(size = 16)) +
    scale_y_continuous(limits = c(0, 0.08), breaks = seq(0, 0.07, 0.02), expand = c(0,0)) +
    scale_fill_manual(values = c("#8da0cb", "grey"), name="Model", labels = c("GLMM", "TMB"))

ggsave("fig4b.tiff", path = "/Users/kluzer01/Desktop/figures/", height = 4, width = 4.5, device='tiff', dpi=700)
```
...
ggsave("fig4d.tiff", path = "/Users/kluzer01/Desktop/figures/", height = 4, width = 10, device='tiff', dpi=700)
